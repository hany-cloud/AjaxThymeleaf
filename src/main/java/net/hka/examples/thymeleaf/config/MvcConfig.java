...
class MvcConfig extends WebMvcConfigurationSupport {
	...
	// -----------------------------------------------------------------------------------------------------------------------------
	// Spring Webflow and Spring MCV configuration and integration with Spring
	// security Section
	// -----------------------------------------------------------------------------------------------------------------------------
	@Bean
	...
	public FlowExecutor flowExecutor() {
		...
		return getFlowExecutorBuilder(this.flowRegistry()) // to set the web flow resource and pattern locations
				.addFlowExecutionListener(new SecurityFlowExecutionListener()) // adding spring security web flow
																				// listener for flow executor
				.build();
	}
	...
	public FlowDefinitionRegistry flowRegistry() {
		...
		return getFlowDefinitionRegistryBuilder().setBasePath(WEBFLOW_RESOURCES_LOCATION)
				.addFlowLocationPattern(WEBFLOW_LOCATION_PATTERN).setFlowBuilderServices(this.flowBuilderServices())
				.build();
	}
	...
	public FlowBuilderServices flowBuilderServices() {
		...
		return getFlowBuilderServicesBuilder() //
				...
				.setValidator(this.getValidator()).build();
	}
	 ...
	 * 
	 * @param flowRegistry the {@link FlowDefinitionRegistry} to configure on the
	 *                     flow executor
	 ...
	 * Return a builder for creating a {@link FlowDefinitionRegistry} instance.
	 * 
	 * @param flowBuilderServices the {@link FlowBuilderServices} to configure on
	 *                            the flow registry with
	 * @return the created builder
	 */
	...
	/*
	 * To apply javax.validation (JSR-303) setup in a Spring application context: It
	...
	/*
	 * Setting the static resources, since we did not using the auto configuration
	...
	/*
	 * Add formatter for class {@link java.util.Date} in addition to the one
	 ...
	 * returned. This is a great idea only in case of the project working in root
	 * context path such as "http://localhost:8080/"
	 */
//    @Controller
